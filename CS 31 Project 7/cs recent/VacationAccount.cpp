...
bool VacationAccount:: redeemVacationAccount(int points,BloodDonation donation){
    if(donation.getID() == getID() && donation.getID()!= -1 && donation.getWeight() != -1 && donation.getAge() != -1 && mID != -1){
        if (points > mBalance){ // do not have enough points
            return false;
        }
        else{
            mBalance = mBalance - points;
            return true; //can donate because they have enough points
        }
    }
    else{
        return false; //return false because they do not have an account
    }
}
